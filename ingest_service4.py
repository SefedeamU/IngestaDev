...
def main():
    ...
    if not table_name or not bucket_name:
        logger.error("Error: DYNAMODB_TABLE_4_DEV y S3_BUCKET_DEV son obligatorios.")
        return
    ...
    if file_format == 'csv':
        ...
        file_name = f'ingest-service-4/{table_name}.csv'  # Guardar en una carpeta específica
    else:
        data = json.dumps(transformed_items, indent=4)
        file_name = f'ingest-service-4/{table_name}.json'  # Guardar en una carpeta específica
    ...
    s3_target = f"s3://{bucket_name}/ingest-service-4/"  # Apuntar a la carpeta específica
